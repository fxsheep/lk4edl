...
static ssize_t elf_read_hook_memory(struct elf_handle *handle, void *buf, uint64_t offset, size_t len) {
    dprintf(DEBUG, "handle %p, buf %p, offset %lld, len %zu\n", handle, buf, offset, len);
...
status_t elf_load(elf_handle_t *handle) {
    ...
    if (!handle->pheaders) {
        dprintf(DEBUG, "failed to allocate memory for program headers\n");
        return ERR_NO_MEMORY;
    }
    ...
    if (readerr < (ssize_t)(handle->eheader.e_phnum * handle->eheader.e_phentsize)) {
        dprintf(DEBUG, "failed to read program headers\n");
        return ERR_NO_MEMORY;
    }

    dprintf(DEBUG, "program headers:\n");
    uint load_count = 0;
    for (uint i = 0; i < handle->eheader.e_phnum; i++) {
        // parse the program headers
        elf_phdr_t *pheader = &handle->pheaders[i];

        dprintf(DEBUG, "%u: type %u offset 0x" ELF_OFF_PRINT_X " vaddr "
        ...
        if (pheader->p_type == PT_LOAD) {
            ...
            if (handle->mem_alloc_hook) {
                ...
                if (err < 0) {
                    dprintf(DEBUG, "mem hook failed, abort\n");
            ...
            if (tozero > 0) {
                ...
                dprintf(DEBUG, "zeroing memory at %p, size %zu\n", ptr2, tozero);
